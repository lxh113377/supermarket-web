...
- **P0（第十六轮开工先做这条）**
- [ ] **M1 rollback 往返判据（两轨分做，障碍二次复核成立）**：结构类 3 件 `DROP COLUMN`＝建列→回滚→断结构变化→只挑"补回该列"那条再前滚→断等于基线；数据类 1 件（`rollback-rename-order20.sql` 是 UPDATE）＝改前/回滚后逐字段行值对账，结构等值对它无意义。
