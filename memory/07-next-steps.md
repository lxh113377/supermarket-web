...
- **P0（待用户点头，勿擅自执行）**：`order 20` 线上品名「猎兽功能饮料**（亏本卖）**」需改。⚠️ **绝不可用 seed 覆盖** —— 实测 D1 该行 `price=1.50`、seed 是 `2.33`，**线上价才是真实售价**，用 seed 同步会把价格改错，属数据破坏。正确动作是只动名称的一次受控写：`UPDATE products SET name='猎兽功能饮料' WHERE "order"=20;`，执行前**必须先加载 `chaoshi-web-deploy`**（红线：任何 d1 execute 前禁裸跑）。前端已用 `line-clamp-2` 兜住长名不撑破卡片。
