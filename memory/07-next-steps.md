...
- **P1**：新画廊放大了**商品图底色不一致**（白底抠图 vs 深色/场景底混排，四列并排深浅方块交错）。根治只能换白底素材或做统一底板 —— 但联网搜图受版权与串图双重限制（见 CHANGELOG 追加十三），且用户已明确不自拍。候选方案：给图区加统一浅灰底板 + 内阴影，让深色底图不那么跳（纯 CSS，零素材成本，可先试）。
...
- **P2**：`order 20` 线上商品名叫「猎兽功能饮料（亏本卖）」，促销语进了正式品名，会在新的大图卡片上被放大展示。
