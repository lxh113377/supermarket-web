...
- **过程中的工具坑（记下来避免误判）**：`grep -n "猎兽" src/data/products-seed.ts` 通过 Bash 传中文模式时**静默返回 0 命中**，紧接第二次用 ASCII 模式加 `-B4` 才看到该串确实在第 61 行。中文 grep 的 0 命中不可直接当"不存在"结论，需换 ASCII 锚点复现一次。
