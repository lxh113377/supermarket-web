...
// 子分类判定沿用现状 drinkSubs：有子分类按集合归属，无子分类归饮品。
...
// ---- 纯 SVG 折线图 ----
function LineChart({ data, labels, color = '#eab308', height = 140 }: { data: number[]; labels: string[]; color?: string; height?: number }) {
  const width = 300
...
// ---- 纯 SVG 环形图 ----
function DonutChart({ segments, size = 140 }: { segments: Array<{ label: string; value: number; color: string }>; size?: number }) {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-100/80 shadow-card animate-fade-in-up stagger-3">
        ...
        {hasReviews ? (
          <LineChart data={reviewTrend.counts} labels={reviewTrend.labels} color="#6366f1" />
        ) : (
      ...
      <div className="bg-white p-5 rounded-2xl border border-gray-100/80 shadow-card animate-fade-in-up stagger-4">
        ...
        {margin.withCostItems === 0 ? (
          <div className="text-center py-6">
            <span className="text-2xl block mb-2">💰</span>
            <p className="text-gray-300 text-sm">待录入</p>
