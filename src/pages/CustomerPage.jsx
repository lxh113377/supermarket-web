...
// ---- 加购飞行动画 ----
function FlyDot({ x, y, tx, ty, onDone }) {
  const endX = tx - 10
  const endY = ty - 10
...
export default function CustomerPage() {
  ...
  const removeDot = (id) => setFlyDots(prev => prev.filter(d => d.id !== id))
  ...
  return (
    <div className="flex flex-col h-full bg-surface">
      ...
      <div className="flex items-center px-4 py-2.5 bg-white/95 backdrop-blur-sm border-b border-gray-100">
        ...
        <span className="ml-2 text-sm font-semibold text-gray-800">零食饮料</span>
      </div>
      <TopNav
